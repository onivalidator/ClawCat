...
class SessionStore:
    ...
    def save_session(self, session) -> bool:
        ...
        if not session.claude_session_id:
            logger.warning("Cannot save session without claude_session_id")
            return False
        ...
        filepath = self._session_file(session.claude_session_id)
        ...
        try:
            ...
            logger.info(f"Session saved: {session.claude_session_id[:8]}")
            return True
    ...
    def load_session_data(self, session_id: str) -> Optional[Dict[str, Any]]:
        ...
        Args:
            session_id: The Claude session ID (full or prefix).
        ...
        for filepath in self.storage_dir.glob("*.json"):
            try:
                with open(filepath, "r", encoding="utf-8") as f:
                    data = json.load(f)
                    if data.get("claude_session_id", "").startswith(session_id):
                        return data
    ...
    def list_sessions(self) -> List[Dict[str, Any]]:
        ...
        for filepath in self.storage_dir.glob("*.json"):
            try:
                ...
                sessions.append({
                    "id": data["claude_session_id"][:8],
                    "full_id": data["claude_session_id"],
